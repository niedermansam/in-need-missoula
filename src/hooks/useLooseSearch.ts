...
export default function useLooseSearch(inputText:string) {
  ...
  const outputArr = inputText.split('').map((letter, letterIndex, letterArray) => (letterIndex < letterArray.length - 1 ? `${inputText.substr(0, letterIndex) + letter}?.?${inputText.substr(letterIndex + 1)}` : ''));

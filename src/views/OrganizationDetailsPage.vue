  ...
  setup(props) {
    ...
    async function loadAllData() {
      ...
      organization.value['Resources Available']
        ...
        .filter(((resource:ResourceSchema) => organization.value['Administer of'] ? organization.value['Administer of'].indexOf(resource.id) === -1 : true))
        : [];
      
      administersList.value = organization.value['Administer of'] ? organization.value['Administer of'].map((id:string) => resourceStore.lookup[id]) : [];

      tagList.value = organization.value['Resources Available'] ? organization.value['Resources Available'].map((resourceId:string) => {
